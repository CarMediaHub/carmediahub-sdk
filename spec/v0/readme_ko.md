...
- `sdk` 필드는 정확한 버전, 캐럿 또는 틸드 semver 범위만 허용합니다. v0 도구는 설치된 SDK 계약 버전 `0.1.0`을 기준으로 호환성을 판단하며 `latest`, 임의 표현식 또는 호환되지 않는 주/부 버전은 허용하지 않습니다.
...
- `components`는 Core가 관리하는 기본 컴포넌트 의존성을 ID, 역할, 선택 여부로 선언합니다. 설치와 업그레이드 시 Core가 카탈로그, 역할, 서명된 설치 기록과 상태를 검증하며 플러그인에는 실행 경로, 명령, 프로세스 또는 호스트 환경 변수를 제공하지 않습니다.
