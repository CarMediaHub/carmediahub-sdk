...
test("worker client rejects a context containing an unknown capability", async () => {
  const address = endpoint();
  const server = net.createServer((socket) => {
    const decoder = new FrameDecoder();
    socket.on("data", (chunk: Buffer) => {
      for (const message of decoder.push(chunk)) {
        const request = message as RpcRequest;
        if (request.method === "broker.hello") socket.write(encodeFrame({ jsonrpc: "2.0", id: request.id, result: { type: "broker.challenge" }, meta: { schemaVersion: "0.1", requestId: request.meta.requestId, traceId: request.meta.traceId } }));
        else if (request.method === "worker.prove") socket.write(encodeFrame({ jsonrpc: "2.0", id: request.id, result: { type: "broker.welcome", context: { scope: { deploymentId: "d", organizationId: "o", userId: "u", deviceId: "device", sessionId: "session", installationId: "plugin" }, locale: "en", timeZone: "UTC", theme: "system", density: "comfortable", entry: "navigation", display: { deviceClass: "unknown", input: [], fullscreenAvailable: false, viewport: { width: 0, height: 0 } }, grantedCapabilities: ["unknown-capability"], policyVersion: 1 } }, meta: { schemaVersion: "0.1", requestId: request.meta.requestId, traceId: request.meta.traceId } }));
      }
    });
  });
  await new Promise<void>((resolve, reject) => { server.once("error", reject); server.listen(address, resolve); });
  try { await assert.rejects(() => connectWorkerClient({ endpoint: address, installationId: "plugin", runtimeCredential: "credential" }), /Broker handshake was denied/u); }
  finally { await new Promise<void>((resolve) => server.close(() => resolve())); if (process.platform !== "win32") fs.rmSync(path.dirname(address), { recursive: true, force: true }); }
});
